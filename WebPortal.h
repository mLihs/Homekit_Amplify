...
constexpr const char * HOSTNAME_DEFAULT = "Amplify";  // -> amplify.local
...
char     wpHostName[HOSTNAME_MAX + 1]        = "Amplify";   // aktiver mDNS-/AP-Basisname
...
.dd-brand{padding:10px 14px 4px;font-size:var(--font-size-body-small);color:var(--text-muted);font-weight:var(--font-weight-semibold);text-transform:uppercase;letter-spacing:.04em;pointer-events:none}
...
<div class="card">
  ...
  <input type="text" id="host" maxlength="24" placeholder="amplify" autocomplete="off">
...
<div class="page-footer">Homekit Amplify</div>
</div>
<script>
...
function loadModels(){fetch('/api/models').then(r=>r.json()).then(d=>{
 ...
 d.models.forEach(function(m,i){
  if(m.b!==lastB){lastB=m.b;var h=document.createElement('div');
   h.className='dd-brand';h.textContent=m.b;l.appendChild(h);}
  var o=document.createElement('div');
  o.className='dd-item'+(i===d.active?' sel':'');
  o.dataset.idx=i;o.textContent=m.n;
  o.onclick=function(){mSel=i;document.getElementById('modelCur').textContent=m.n;
   l.querySelectorAll('.dd-item').forEach(function(el){el.classList.toggle('sel',+el.dataset.idx===i);});
   ddSet(false);};
  l.appendChild(o);});
 var cur=d.models[d.active];document.getElementById('modelCur').textContent=cur?cur.n:'–';
}).catch(()=>{});}
function doModel(){
...
var pend={},pt={},hold={};
function volMin(){return (S&&typeof S.volMin==='number')?S.volMin:0}
function volMax(){return (S&&typeof S.volMax==='number')?S.volMax:96}
function cur(k){return k=='vol'?S.vol:k=='bass'?S.bass:k=='treb'?S.treble:S.bal}
function lim(k){return k=='vol'?[volMin(),volMax()]:k=='bal'?[-S.balMax,S.balMax]:[-10,10]}
...
function paint(k,v){if(k=='vol'){var f=el('volFill');var mn=volMin(),mx=volMax(),span=Math.max(mx-mn,1);
 f.style.width=Math.max((v-mn)/span*100,14)+'%';f.textContent=v;}
...
/* Bass/Treble rasten geraeteabhaengig (NAD nur gerade Werte) */
function stepSize(k){return (k=='bass'||k=='treb')?((S&&S.toneStep)||1):1}
function step(k,d){if(S)setv(k,(k in pend?pend[k]:cur(k))+d*stepSize(k));}
...
function render(d){
 ...
 else{el('cardTone').style.display='';el('grpBal').style.display=(d.balMax>0)?'':'none';}
 var off=!d.power;
...
// Die TX-Queue von ampCallback ist bewusst Single-Producer (Haupt-Loop).
// HTTP-Handler legen Aktionen deshalb in diesen kleinen SPSC-Ring; erst
// webPortalLoop() ruft die _amp-Methoden auf. Byte-Indizes, keine Sperren.
...
static void registerDashboardRoutes(){
  ...
  webServer.on("/api/state", HTTP_GET, [](AsyncWebServerRequest *req){
    ...
    size_t pos = snprintf(json, sizeof(json),
      "{\"power\":%s,\"vol\":%d,\"volMin\":%d,\"volMax\":%d,\"src\":%d,\"mute\":%s,\"bypass\":%s,"
      "\"bass\":%d,\"treble\":%d,\"toneStep\":%d,\"bal\":%d,\"balMax\":%d,"
      "\"hasTone\":%s,\"powerAmp\":%s,\"fw\":\"%s\",\"sources\":[",
      _amp._currentPower ? "true" : "false",
      _amp._currentVolume, (int)ampModel->volumeMin, (int)ampModel->volumeMax,
      _amp._currentSource,
      _amp._currentMute ? "true" : "false",
      _amp._currentBypass ? "true" : "false",
      _amp._currentBass, _amp._currentTreble, (int)_amp.toneStep(),
      _amp._currentBalance,
      (int)ampModel->balanceMax,
      ampModel->hasTone ? "true" : "false",
      (ampModel->deviceType == AMP_POWER_AMP) ? "true" : "false",
      wymrfirmware);
    for (uint8_t i = 0; i < ampModel->sourceCount && pos < sizeof(json); i++)
      pos += snprintf(json + pos, sizeof(json) - pos, "%s\"%s\"",
                      i ? "," : "", ampModel->sources[i].name);
  ...
  webServer.on("/api/cmd", HTTP_POST, [](AsyncWebServerRequest *req){
    ...
    if (action == WR_SOURCE) {
      ...
      if (id < 1 || id > (int)ampModel->sourceCount) {
      ...
      const int lo = (action == WR_VOL_SET) ? (int)ampModel->volumeMin
                   : (action == WR_BAL_SET) ? -(int)ampModel->balanceMax : -10;
      const int hi = (action == WR_VOL_SET) ? (int)ampModel->volumeMax
                   : (action == WR_BAL_SET) ? (int)ampModel->balanceMax : 10;
  ...
  webServer.on("/api/info", HTTP_GET, [](AsyncWebServerRequest *req){
    ...
    const char *src = "-";
    if (_amp._currentSource >= 1 && _amp._currentSource <= (int)ampModel->sourceCount)
      src = ampModel->sources[_amp._currentSource - 1].name;
    snprintf(json, sizeof(json),
      ...
      ampModel->name, _amp.generationLabel(), wpHostName,
      _amp._currentPower ? "true" : "false", src,
      (unsigned long)ESP.getFreeHeap());
    req->send(200, "application/json", json);
  });

  webServer.on("/api/models", HTTP_GET, [](AsyncWebServerRequest *req){
    // Modellliste mit Marke; Namen ASCII ohne Anfuehrungszeichen.
    // ~25 Modelle a ~60 B -> 2048 B reichen sicher.
    static char json[2048];
    size_t pos = snprintf(json, sizeof(json), "{\"active\":%u,\"models\":[",
                          (unsigned)(ampModel - AMP_MODELS));
    for (uint8_t i = 0; i < AMP_MODEL_COUNT && pos < sizeof(json); i++) {
      pos += snprintf(json + pos, sizeof(json) - pos,
                      "%s{\"n\":\"%s\",\"b\":\"%s\",\"v\":%s}",
                      i ? "," : "",
                      AMP_MODELS[i].name,
                      AMP_MODELS[i].brand,
                      AMP_MODELS[i].verified ? "true" : "false");
    }
    if (pos < sizeof(json)) pos += snprintf(json + pos, sizeof(json) - pos, "]}");
    req->send(200, "application/json", json);
  });
  ...
  webServer.on("/api/model", HTTP_POST, [](AsyncWebServerRequest *req){
    ...
    if (idx < 0 || idx >= (int)AMP_MODEL_COUNT) {
...
void webPortalSetup(){
  ...
  wmlPortal.setDeviceName("Homekit Amplify");
...
void webPortalLoop(){
  ...
  while (wrTail != wrHead) {
    ...
    switch (action) {
      case WR_POWER_ON:   _amp.setPower(true);         break;
      case WR_POWER_OFF:  _amp.setPower(false);        break;
      case WR_SOURCE:     _amp.setSource(param);       break;
      case WR_MUTE:       _amp.toggleMute();           break;
      case WR_BYPASS:     _amp.toggleBypass();         break;
      case WR_VOL_SET:    _amp.setVolumeNN(param);     break;
      case WR_BASS_SET:   _amp.setBassValue(param);    break;
      case WR_TREBLE_SET: _amp.setTrebleValue(param);  break;
      case WR_BAL_SET:    _amp.setBalanceValue(param); break;
      default: break;
    }
  }
  ...
  if (wpModelRequested && (millis() - wpModelRequestAt >= RESET_DELAY_MS)) {
    ...
    ampSaveModelIndex(wpPendingModel);
